...
class FileFilterTest
{
    ...
    {
        new FileFilter('/etc/does/not/exist/at/all');
    }

    /**
     * @Expect("Demeanor\Exception\InvalidArgumentException")
     */
    public function testInstantiationThrowsExceptionWhenThePathIsNotAFile()
    {
        new FileFilter(__DIR__);
    }

    public function testTestCaseIsRunnableWhenATestCaseIsInTheSuppliedFile()
    {
        $tc = $this->testCaseInFile(__FILE__);
        $filter = new FileFilter(__FILE__);

        $this->assertTrue($filter->canRun($tc));
    }

    public function testTestCaseIsNotRunnableWhenNotInSuppliedFile()
    {
